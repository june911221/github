...
<link rel="stylesheet" href="css/food.css?ver=1.1">
...
<body>
	...
	<section>
		...
		<div class="container">
			...
			<img id="title" src="img/food_title.png">
		 <!-- 검색창 -->
		<form action="food.jsp?category1=맛집" method="post">
		<div id="searchbar">
...
if(request.getParameter("searchbox")==null){//검색어를  request.getParameter("searchbox") 를통해 받아서 입력하지않았을 일반적인 상황인경우 정상적으로 리스트출력
...
else{//검색어가 입력이 들어오면 검색어에해당하는 내용으로 리스트출력
				...
				<div class="wrapper2">
					...
					<div class="product-info">
						...
						<div class="product-price-btn">
							<a href="placedetail.jsp?num=<%=view.get(i).getNum()%>">
...
<%if(nowPage<=1){ %>
		...
		<%}else{ %>
			<li class="page-item"><a id="page-id2" href="place.jsp?category1=맛집&page=<%=nowPage-1%>">[이전]</a></li>&nbsp;
		<%} %>

		<%for(int a=startPage;a<=endPage;a++){
				if(a==nowPage){%>
		<li class="page-item"><a id="page-id" href="place.jsp?category1=맛집&page=<%=a%>"><%=a%></a></li>
		<%}else{ %>
			<li class="page-item"><a id="page-id" href="place.jsp?category1=맛집&page=<%=a%>"><%=a%></a></li>&nbsp;
		<%} %>
		<%} %>
		...
		<%}else{ %>
			<li class="page-item"><a id="page-id2" href="place.jsp?category1=맛집&page=<%=nowPage+1%>">[다음]</a></li>
		<%} %>
		</ul>
	</div>
	<%
