...
function whenSuccess(resdata) {
	if (resdata == 1) {
		...
		$("#chid").val("");// 중복체크를 했는지 확인하기위한 요소로 기본적인 값이 셋팅되어있고 사용가능하면 값을 ""로
							// 바꿔서 전체 폼체크시 값이 ""이면 중복체크를 한것으로 간주
	}
}
function whenError() {
	alert("Error");
...
function Success(resdata) {
	...
	if (resdata == 1) {
		...
		$("#chmail").val("");// 중복체크를 했는지 확인하기위한 요소로 기본적인 값이 셋팅되어있고 사용가능하면 값을
								// ""로 바꿔서 전체 폼체크시 값이 ""이면 중복체크를 한것으로 간주
	}
...
function check() {
	...
	// ID중복확인 버튼 눌렀는지확인
	if ($("#chid").val() != "") { // 버튼을누르면 #chid value값이 "" 가되는데 ""이아닐경우
									// 버튼을누른게 아니므로 누르라고 경고창
		alert("ID 중복확인을 해주세요");
		return false;
	}

	// email중복확인 버튼 눌렀는지확인
	if ($("#chmail").val() != "") {// 버튼을누르면 #chmail value값이 "" 가되는데 ""이아닐경우
									// 버튼을누른게 아니므로 누르라고 경고창
